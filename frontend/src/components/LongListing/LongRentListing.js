...
const LongRentListing = () => {
  const {rentListings, currentPage, listingsPerPage, } = useContext(ContextAPI);

  const indexOfLastListing = currentPage * listingsPerPage;
  const indexOfFirstListing = indexOfLastListing - listingsPerPage;
  const listings = rentListings.slice(indexOfFirstListing, indexOfLastListing);
...
export default LongRentListing;
